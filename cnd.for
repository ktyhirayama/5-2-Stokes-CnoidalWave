...
C
C
      SUBROUTINE CND(N1,D1,T,H,RL,C1)
C
C------------------------------------------------
C  List of Symbols
C    N1 (i)  = (I) Order of the theory  (<=3)
C    D1 (f)  = (I) Mean water depth     (m)
C    T  (f)  = (I) Wave period          (s)
C    H  (f)  = (I) Wave height          (m)
C    RL (f)  = (O) Wavelength           (m)
C    C1 (f)  = (O) Phase velocity       (m/s)
C------------------------------------------------
C
      DIMENSION A(4),B(3,4),BB(3,4)
      SAVE
      DATA G,PI/9.80,3.141593/, EPS/1.E-5/
C
      X=1.E10
      N=N1
      NN=N+1
...
C
C
      SUBROUTINE ELINT(QL1,RK1,RKK1,SK1,SKK1,SE1,SEE1)
C
      SAVE
